...
<script>
const apiKey = "8f73999d6493a59142d8c107c1ada126";
...
// Initialize Camera - Using modern promise-based API
function initCamera() {
    navigator.mediaDevices.getUserMedia({ video: { facingMode: "user" } })
    .then(stream => { 
        video.srcObject = stream;
        // Start the 2-second capture loop once the camera is ready
        if (!captureTimer) {
...
async function uploadImage() {
    // Only upload if the game is running and video is playing
...
function startGame() {
    ...
    initCamera(); // Ensure camera is active
    gameLoop();
}
